...
2. 在“**发现服务器**”页面，输入 Discovery UR 后，点击搜索按钮，显示当前 URL 下所有 OPC UA Server 列表。
...
5. 单击“**确认**”按钮。此时该条数据将显示在 OPC UA 的设备列表页面。
...
1. 按照上方示例的步骤创建OPC UA设备和组。
2. 在操作栏中点击修改按钮，在认证页面，点击“测试连接“按钮。

      ![alt text](12.png)

3. 连接将显示为“**测试连接失败**”。这通常是正确且预期的，因为 KEPServerEX 拒绝访问系统第一次创建的 OPC UA 客户端。因此只需要让 KEPServerEX 信任系统创建的 OPC UA 客户端。
