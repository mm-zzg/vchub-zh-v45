...
在工业现场，往往需要批量创建设备。WAGO VC Hub 通过导出和导入功能来实现这一功能。
...
- 红框中内容为字段信息。
- 如果是设备，则“IsDevice”字段为 **True**。
- 组信息紧跟在对应的设备下面。例如上图的设备“**Device1**”下，存在1个组，Group。

**说明**：如果OPC UA设备使用 **用户名，密码** 进行身份验证，导出的时候不包含这两个字段。导入设备后，需手动为每个设备设置用户名和密码。
