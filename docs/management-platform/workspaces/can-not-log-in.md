...
3. **进入工程文件夹\Workspaces**
      ...
      - 该目录下包含了所有工程，每个工程存储在独立的文件夹中。使用工作区 ID 作为文件夹名称。

        ![alt text](21.png)

      - 打开每个工作区文件夹，并检查 .ini 文件中的工作区名称。找到您知道用户名和密码的工作区，并记录其 ID。   
...
2. **进入程序安装目录**
      ...
      - Windows 系统的默认安装目录：`"C:\Program Files\WAGO Visualization And Control Hub" `
      - Linux 系统的默认安装目录：
      `"/usr/local/bin/wagovisualizationandcontrolhub-x.x.x-linux-x64"`

      - 如果您安装时修改过安装目录，请进入您修改的安装目录。

3. **在当前版本目录中，导航到 MarkFiles 文件夹并删除 workspace_initialized 文件。**
      ![alt text](24.png)
4. **重启服务**
